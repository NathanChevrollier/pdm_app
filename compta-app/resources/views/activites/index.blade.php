...
<div class="row mt-4">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between mb-3">
                    ...
                    <span class="fw-bold">{{ isset($stats) ? $stats['total'] ?? 0 : 0 }}</span>
                </div>
                <div class="d-flex justify-content-between mb-3">
                    <span>Connexions:</span>
                    <span class="fw-bold">{{ isset($stats) ? $stats['connexions'] ?? 0 : 0 }}</span>
                </div>
                <div class="d-flex justify-content-between mb-3">
                    <span>Créations:</span>
                    <span class="fw-bold">{{ isset($stats) ? $stats['creations'] ?? 0 : 0 }}</span>
                </div>
                <div class="d-flex justify-content-between mb-3">
                    <span>Modifications:</span>
                    <span class="fw-bold">{{ isset($stats) ? $stats['modifications'] ?? 0 : 0 }}</span>
                </div>
                <div class="d-flex justify-content-between">
                    <span>Suppressions:</span>
                    <span class="fw-bold">{{ isset($stats) ? $stats['suppressions'] ?? 0 : 0 }}</span>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-6">
        <div class="card">
            ...
            <div class="card-body">
                <ul class="timeline">
                    @if(isset($recentes) && count($recentes) > 0)
                        @foreach($recentes as $recente)
                            <li class="timeline-item mb-3">
                                <span class="timeline-point timeline-point-{{ $recente->type == 'connexion' ? 'primary' : ($recente->type == 'creation' ? 'success' : ($recente->type == 'modification' ? 'info' : ($recente->type == 'suppression' ? 'danger' : 'warning'))) }} timeline-point-indicator"></span>
                                <div class="timeline-event">
                                    <div class="timeline-header">
                                        <h6 class="mb-0">{{ $recente->user ? $recente->user->nom . ' ' . $recente->user->prenom : 'Système' }}</h6>
                                        <small class="text-muted">{{ \Carbon\Carbon::parse($recente->created_at)->diffForHumans() }}</small>
                                    </div>
                                    <p class="mb-0">{{ $recente->description }}</p>
                                    @switch($recente->type)
                                        @case('connexion')
                                            <span class="badge bg-primary" style="background: linear-gradient(45deg, #696cff, #8592d8) !important;">Connexion</span>
                                            @break
                                        ...
                                        @case('paiement')
                                            ...
                                            @break
                                        @default
                                            <span class="badge bg-secondary" style="background: linear-gradient(45deg, #a8aaae, #c6c8cd) !important;">{{ ucfirst($recente->type) }}</span>
                                    @endswitch
                                </div>
                            </li>
                        @endforeach
                    @else
                        <li class="text-center">Aucune activité récente</li>
                    @endif
                </ul>
            </div>
        </div>
